...
if not os.path.exists("PlotsCompare"):
    os.makedirs("PlotsCompare")
...
for wp in writes_percentage:
    for opt in objs_per_transaction:
        for nohte in number_of_ht_entries:
            ...
            ax.set_title(f"DHT\nNumber of HT entries: {nohte}, Percentage of writes: {wp} %, Objects per transaction:{opt}", fontsize=24)
            ...
            fig.set_figwidth(19)
            fig.savefig(f"PlotsCompare/NKEYS: {number_of_keys}, NHTENTRIES: {nohte}, WRITES: {wp}, NOBJTRANS:{opt}.png")
            n+=1
            i+=1
