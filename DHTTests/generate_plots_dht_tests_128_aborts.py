...
for cm in contention_managers:
    dht_tests_filename = "tests_results/dht_" + cm.lower() + "_128.txt"
                ...
                for noc in number_of_clients:
                    for test in range(number_of_tests):
                        ...
                        test_cases_dict[f"NKEYS: {number_of_keys}, NHTENTRIES: {nohte}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc].append(
                            int(aborts[i].split("Total of aborts: ")[1]))
                        i+=1
...
same_contention_aborts = {number_of_ht_entries[0]: 0, number_of_ht_entries[1]: 0}
same_contention_same_client_aborts = {number_of_ht_entries[0]: {noc: 0 for noc in number_of_clients}, 
        ...
        for nohte in number_of_ht_entries:
            for cm in contention_managers:
                for noc in number_of_clients:
                    ...
                    same_contention_aborts[nohte] += avg
                    same_contention_same_client_aborts[nohte][noc] += avg
...
if not os.path.exists("Plots128"):
    os.makedirs("Plots128")
if not os.path.exists("Plots128/Aborts"):
    os.makedirs("Plots128/Aborts")
...
for wp in writes_percentage:
    for opt in objs_per_transaction:
        for nohte in number_of_ht_entries:
            ...
            ax.set_ylabel('Aborts', fontsize=20)
            ...
            fig.set_figwidth(19)
            fig.savefig(f"Plots128/Aborts/NKEYS: {number_of_keys}, NHTENTRIES: {nohte}, WRITES: {wp}, NOBJTRANS:{opt}.png")
            n+=1
            i+=1

print(n)

print("HIGH CONTENTION ABORTS AVERAGE: " + str(same_contention_aborts[number_of_ht_entries[0]] / (4 * 4 * 8)))
print("LOW CONTENTION ABORTS AVERAGE: " + str(same_contention_aborts[number_of_ht_entries[1]] / (4 * 4 * 8)))

for noc in number_of_clients:
    print(str(noc) + " CLIENTS; HIGH CONTENTION ABORTS AVERAGE: " + str(same_contention_same_client_aborts[number_of_ht_entries[0]][noc] / (4 * 8)))
    print(str(noc) + " CLIENTS; LOW CONTENTION ABORTS AVERAGE: " + str(same_contention_same_client_aborts[number_of_ht_entries[1]][noc] / (4 * 8)))
