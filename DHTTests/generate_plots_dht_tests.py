...
contention_managers = ["Agressive", "Less", "Karma", "Kindergarten", "Passive", "Polite", "Polka", "Timestamp"]
...
number_of_keys = [1000, 10000]
number_of_ht_entries = {1000: 128, 10000: 97} #[128, 97]
...
for wp in writes_percentage:
    for opt in objs_per_transaction:
        for nok in number_of_keys:
            test_cases_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"] = {
...
for wp in writes_percentage:
    for opt in objs_per_transaction:
        for nok in number_of_keys:
            test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"] = {
        ...
        for nok in number_of_keys:
            for cm in contention_managers:
                for noc in number_of_clients:
                    ...
                    max_value = -1
                    for test in range(number_of_tests):
                        avg += test_cases_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc][test]
                        max_value = max(max_value, test_cases_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc][test])
                        min_value = min(min_value, test_cases_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc][test])
                    avg /= number_of_tests*1000
                    min_value /= 1000
                    max_value /= 1000
                    test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["avg"] = round(avg, 2)
                    test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["min"] = round(min_value, 2)
                    test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["max"] = round(max_value, 2)
                    test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["lower_bound"] = round(avg - min_value, 2)
                    test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["upper_bound"] = round(max_value - avg, 2)
    ...
    for opt in objs_per_transaction:
        for nok in number_of_keys:
            for cm in contention_managers:
                ...
                upper_bound = []
                for noc in number_of_clients:
                    count.append(test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["avg"])
                    min_value.append(test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["min"])
                    max_value.append(test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["max"])
                    lower_bound.append(test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["lower_bound"])
                    upper_bound.append(test_cases_avgs_dict[f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}"][cm][noc]["upper_bound"])
                    j += 1
                #print(count)
...
for wp in writes_percentage:
    for opt in objs_per_transaction:
        for nok in number_of_keys:
            ...
            print(f"NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, Percentage of writes: {wp} %, Objects per transaction:{opt}")
            for cm in contention_managers:
                print(counts[i][j])
            ...
            ax.set_title(f"DHT\nNumber of keys: {nok}, Number of HT entries: {number_of_ht_entries[nok]}, Percentage of writes: {wp} %, Objects per transaction:{opt}", fontsize=20)
            ...
            fig.set_figwidth(19)
            fig.savefig(f"Plots/NKEYS: {nok}, NHTENTRIES: {number_of_ht_entries[nok]}, WRITES: {wp}, NOBJTRANS:{opt}.png")
            n+=1
            i+=1
