...
same_contention_aborts = {100: 0, 500: 0}
same_contention_same_client_aborts = {100: {noc: 0 for noc in number_of_clients}, 500: {noc: 0 for noc in number_of_clients}}
        ...
        for ops in objs_per_server:
            for cm in contention_managers:
                for noc in number_of_clients:
                    ...
                    same_contention_aborts[ops] += avg
                    #print(same_contention_same_client_aborts[ops][noc])
                    same_contention_same_client_aborts[ops][noc] += avg
...
if not os.path.exists("PlotsNew/Aborts"):
    os.makedirs("PlotsNew/Aborts")
...
for wp in writes_percentage:
    for opt in objs_per_transaction:
        for ops in objs_per_server:
            ...
            fig.set_figwidth(19)
            fig.savefig(f"PlotsNew/Aborts/NOBJSERVER_{ops}, WRITES: {wp}, NOBJTRANS:{opt}.png")
            n+=1
            i+=1

print(n)

print("HIGH CONTENTION ABORTS AVERAGE: " + str(same_contention_aborts[100] / (4 * 4 * 8)))
print("LOW CONTENTION ABORTS AVERAGE: " + str(same_contention_aborts[500] / (4 * 4 * 8)))

for noc in number_of_clients:
    print(str(noc) + " CLIENTS; HIGH CONTENTION ABORTS AVERAGE: " + str(same_contention_same_client_aborts[100][noc] / (4 * 8)))
    print(str(noc) + " CLIENTS; LOW CONTENTION ABORTS AVERAGE: " + str(same_contention_same_client_aborts[500][noc] / (4 * 8)))
