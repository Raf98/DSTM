...
for cm in contention_managers:
    ...
    if cm == "Passive":
        dht_tests_filename = f"../../GenericBenchFinalTests/tests_results/{cm.lower()}_final_tests_results_new.txt"
...
if not os.path.exists("Plots"):
    os.makedirs("Plots")
if not os.path.exists("Plots/pt-br"):
    os.makedirs("Plots/pt-br")
...
for wp in writes_percentage:
    for opt in objs_per_transaction:
        for ops in objs_per_server:
            ...
            ax.set_ylabel('Tempo (segundos)', fontsize=24)
            ax.set_xlabel('Número de clientes', fontsize=24)
            ax.set_title(f"Objetos por servidor: {ops}, Porcentagem de escritas: {wp} %, Objetos por transação:{opt}", fontsize=24)
            ...
            fig.set_figwidth(19)
            fig.savefig(f"Plots/pt-br/NOBJSERVER_{ops}, WRITES: {wp}, NOBJTRANS:{opt}.png")
            n+=1
            i+=1
