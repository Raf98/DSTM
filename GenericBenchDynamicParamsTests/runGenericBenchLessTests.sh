    ...
    do
        ...
        do
            ...
            do
                ...
                maxSelfAborts=128
                while [[ $maxSelfAborts -le 2048 ]];
                do
                    for i in $(seq 0 9);
                    do
                        echo "Test $i for TRMILess: $maxSelfAborts maxSelfAborts"
                        printf "TRMILess\t$NCLIENT\t"
                        ./runGenericBench_CMsParams.sh $NSERVER $NOBJSERVER $NCLIENT $WRITES $NTRANS $NOBJTRANS 6 $maxSelfAborts
                    done
                    let "maxSelfAborts*=2" #32 64 128
                done
                let "NCLIENT*=2"
            done
        done
