...
class GenericTransaction implements ExecTransaction {
   ...
   public void execTransaction(RObject[] objects, int op, int contentionManager) throws Exception {
      ...
      for (int i = 0; i < robjects.length; i++)
         robjects[i] = (IObject) Naming
               .lookup("rmi://localhost:" + objects[i].getPort() + "/" + objects[i].getAddress());
      ...
      if (op == 0) {
         ...
         for (int i = 0; i < size; i += 2) {
            ...
            if (result1 >= 100) {
               robjects[i].getClass().getDeclaredMethod("setField" + field1, int.class).invoke(robjects[i],
                     result1 - 100);
               result2 = (int) robjects[i + 1].getClass().getDeclaredMethod("getField" + field2)
                     .invoke(robjects[i + 1]);
               robjects[i + 1].getClass().getDeclaredMethod("setField" + field2, int.class).invoke(robjects[i + 1],
                     result2 + 100);
            }
