...
class GenericTransaction implements ExecTransaction {
  ...
  public void execTransaction(RObject[] objects, int op, int contentionManager) throws Exception {
    ...
    for (int i = 0; i < robjects.length; i++) {
      ...
      robjects[i] = (TMObj<IObject>) TMObj
          .lookupTMObj("rmi://localhost:" + objects[i].getPort() + "/" + objects[i].getAddress());
    }
      ...
      public Integer call() throws Exception {
        ...
        if (op == 0) {
          ...
          for (int i = 0; i < size; i += 2) {
            ...
            if (result1 >= 100) {
              lobjects[i].getClass().getDeclaredMethod("setField" + field1, int.class).invoke(lobjects[i],
                  result1 - 100);
              result2 = (int) lobjects[i + 1].getClass().getDeclaredMethod("getField" + field2).invoke(lobjects[i + 1]);
              lobjects[i + 1].getClass().getDeclaredMethod("setField" + field2, int.class).invoke(lobjects[i + 1],
                  result2 + 100);
            }
